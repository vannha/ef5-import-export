 ...
 * @param: $file
 */
function ef5_ie_options_export($file)
{
    ...
    /* default. */
    $options['home'] = ef5_ie_options_get_home_page();
    $options['menus'] = ef5_ie_options_get_menus();
    $options['opt-name'] = ef5_ie_setting_get_opt_name($file);
    $options['export'] = !empty($_POST['ef5-ie-data-type']) ? $_POST['ef5-ie-data-type'] : array();

    /* wp options */
    $options['wp-options'] =ef5_ie_options_get_wp_options(apply_filters('ef5_ie_options_wp_options', array()));
...
function ef5_ie_options_get_home_page()
{
...
function ef5_ie_options_get_menus()
{
...
function ef5_ie_options_get_wp_options($options = array())
{
    if (empty($options))
        return $options;
 ...
 * @param $options
 */
function ef5_ie_options_import($options)
{
    global $import_result;
    foreach ($options as $key => $option) {
        switch ($key) {
            case 'home':
                ef5_ie_options_set_home_page($option);
                $import_result[] = esc_html__('Set home page successfully!', 'ef5-ie');
                break;
            case 'menus':
...
function ef5_ie_options_set_home_page($slug)
{
...
function ef5_ie_options_set_menus($menus)
{
    if (!empty($menus)) {
        $new_setting = array();
...
function ef5_ie_options_set_wp_options($options = array())
{
    if (empty($options))
        return;
